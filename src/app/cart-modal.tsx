...
export default function CartModal() {
  ...
  return (
    <Drawer open={isCartOpen} onOpenChange={closeCart} direction="right">
      <DrawerContent className="fixed top-0 right-0 h-full max-w-md w-full rounded-none p-0 flex flex-col shadow-2xl border-l bg-background z-50 animate-in slide-in-from-right-32" style={{ maxWidth: 400 }}>
        ...
        <DrawerHeader className="px-6 py-4 border-b border-border">
          <div className="flex items-center justify-between w-full">
            ...
            <DrawerClose asChild>
              <button onClick={closeCart} aria-label="Close" className="p-2 hover:bg-muted rounded-full transition-colors">
                <X className="w-8 h-8 text-foreground font-bold" />
              </button>
            </DrawerClose>
          </div>
        </DrawerHeader>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {cart.length === 0 ? (
            <div className="text-center py-8 text-foreground">Your cart is empty.</div>
          ) : (
            <div>
              {cart.map((item) => (
                <div key={item.id} className="flex items-center gap-4 mb-6">
                  <div className="w-20 h-20 flex items-center justify-center flex-shrink-0 bg-card rounded">
                    <div className="relative w-16 h-16">
                      <ThumbnailImage src={item.image_url || '/placeholder.png'} alt={item.name} fill style={{ objectFit: 'contain' }} className="rounded" />
                    </div>
                  </div>
                  <div className="flex-1 flex flex-col justify-center min-h-[80px] text-foreground">
              ...
              <div className="flex justify-between items-center font-bold text-lg mt-2 mb-6 text-foreground">
          ...
          {isProductsLoading ? (
            <div className="mt-6">
              <div className="font-extrabold text-lg mb-4 uppercase text-foreground">You May Also Like</div>
